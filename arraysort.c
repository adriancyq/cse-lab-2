...
Description:    Implementation of a sorted array-based list. Allows for 
                insertion, deletion, searching, popping, and printing.
****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
{
    void * doubledList;                 /* Pointer to doubled list */
    int tempNum;                        /* Used for switching numbers */
    ...
    if ((ls->size) == (ls->maxSize)) {
        doubledList = realloc((void *)(ls->sortedList), 
            (ls->maxSize) * 2 * sizeof(int));
